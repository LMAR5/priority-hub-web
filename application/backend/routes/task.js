...
const {getalltasks, getdeltasks, getcomptasks, searchtasks} =
    require('./taskService');
...
// Type: GET
// Description: Search a task by its name or other fields sending a keyword from
// frontend
...
router.post('/CreateTask', async (request, response) => {
  ...
  const createQuery =
      'INSERT INTO Task (Title, Description, CategoryID, UserID, Status, Priority, DueDate, Notes, CreatedBy, CreatedDateTime, LastUpdatedBy, LastUpdatedDateTime) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)';
  const createValue = [
    TaskName, Description, Category, 1, 'Pending', 'Low', DueDate, Notes,
...
// Type: PUT
// Description: Update the task based on the values receive from the HTTP
// request body
  ...
  db.query(selectQuery, selectValues, async (error, result) => {
    if (error) {
      return response.status(500).json({
        message: 'Error when creating record. Your time has not been recorded.',
        success: false
      });
    }
    if (result.length === 1) {
      const newres = await db.promise().query(
          `UPDATE Task SET Status='In Progress' WHERE Id='${taskid}'`);
